...
class TestDBreconnection(unittest.TestCase):
    ...
    def test_init_connections_failure(
        self, mock_print, mock_saving, mock_event, mock_book
    ):
        ...
        mock_print.assert_called_with(
            "Failed to initialize database connections: Connection failed"
        )
    ...
    async def test_dbreconnect_success(
        self,
        mock_getenv,
        mock_load_dotenv,
        mock_saving,
        ...
        with patch("Cogs.DBreconnection.e.get_embed") as mock_embed:
            ...
            mock_embed.assert_called_with(
                "✅ 데이터베이스 재연결 성공!", title="재연결 완료"
            )
            interaction.followup.send.assert_called_once_with(embed=mock_embed_obj)

    @patch(
        "Cogs.DBreconnection.psycopg2.connect",
        side_effect=psycopg2.OperationalError("Connection refused"),
    )
    @patch("Cogs.DBreconnection.load_dotenv")
    @patch("Cogs.DBreconnection.os.getenv")
    async def test_dbreconnect_operational_error(
        self, mock_getenv, mock_load_dotenv, mock_connect
    ):
        ...
        with patch("Cogs.DBreconnection.e.get_embed") as mock_embed:
            ...
            mock_embed.assert_called_with(
                "🐱 재연결 실패! 서버 관리자에게 문의하세요", error=True
            )
    ...
    @patch("Cogs.DBreconnection.load_dotenv")
    @patch("Cogs.DBreconnection.os.getenv")
    async def test_dbreconnect_updates_other_cogs(
        self,
        mock_getenv,
        mock_load_dotenv,
        mock_saving,
