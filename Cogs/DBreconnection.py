...
class DBreconnection(commands.Cog):
    ...
    async def dbreconnect(self, interaction: discord.Interaction):
            ...
            for cog_name in ["BookRetrieval", "Sql", "Event", "Saving"]:
                ...
                if cog:
                    ...
                    if (
                        hasattr(cog, "event")
                        and self.event
                        and self.event.is_connected()
                    ):
                        cog.event = self.event
                    if (
                        hasattr(cog, "saving")
                        and self.saving
                        and self.saving.is_connected()
                    ):
        ...
        if success_count == 3:
            ...
            embed = embedding.get_embed(
                "✅ 모든 데이터베이스 연결이 성공적으로 재연결되었습니다!",
                title="재연결 완료",
            )
            embed.add_field(
                name="재연결된 핸들러",
            ...
            # Partial success
            embed = embedding.get_embed(
                "⚠️ 일부 데이터베이스 연결이 재연결되었습니다", title="부분 재연결"
            )
            ...
            # Complete failure
            embed = embedding.get_embed(
                "🐱 모든 데이터베이스 재연결이 실패했습니다! 서버 관리자에게 문의하세요",
                error=True,
            )
            if failed_handlers:
                embed.add_field(
