...
def get_next_word_from_api(last_letter):
    ...
    if "channel" in data and "item" in data["channel"]:
        words = [item["word"].replace('-', '') for item in data["channel"]["item"]
...
class ForFun(commands.Cog):
    ...
    async def who_ru(self, interaction: discord.Interaction, member: discord.Member):
        await interaction.response.send_message(f"{member.mention} 넌 누구냐 !!!! :gun:")
    ...
    async def houraisan_kaguya(self, interaction: discord.Interaction):
        urllib.request.urlretrieve(
            "https://media1.tenor.com/m/fV1krxBGQWAAAAAd/kaguya-houraisan-kaguya-houraisan-exploding.gif", "kaguya.gif")
    ...
    async def eng(self, interaction: discord.Interaction, num: int):
        if abs(num) > 1000:
            await interaction.response.send_message(f"{interaction.user.mention} 범위를 넘어갔어요! :sob:")
            return
    ...
    async def shiritori(self, interaction: discord.Interaction, word: str):
        ...
        if word.lower() == 'q':
            used_words.clear()
            await interaction.response.send_message(f"초기화됐습니다! 먼저 시작하세여 <a:InuiWaveYay:1272086403135832125>")
            return
        elif word in used_words:
            tag1 = '은' if has_final_consonant(word[0]) else '는'
            await interaction.response.send_message(f"'{word}'{tag1} 이미 쓴 단어라구요 <a:InuiWaveYay:1272086403135832125>")
            return
            ...
            await interaction.response.send_message(f"'{word[0]}'{tag1} 아니라 {used_words[-1][-1]}{tag2} 끝냈거든요 !")
            return
        elif len(word) == 1:
            await interaction.response.send_message(f"'{word}'는 한글자거든요 <a:InuiWaveYay:1272086403135832125>")
            return
