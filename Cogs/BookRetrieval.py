...
category_list = [app_commands.Choice(name="만화", value="만화"),
...
language_list = [app_commands.Choice(name="한국어", value="kor"),
...
change_list = [app_commands.Choice(name="도서명", value="title"),
...
class BookRetrieval(commands.Cog):
    ...
    async def search_books(self, interaction: discord.Interaction, title: str, series: str = None):
        # Exception
    ...
    async def rent_book_outer(self, interaction: discord.Interaction, name: discord.Member, book_id: str):
        try:
            if interaction.channel_id not in (1280321483285074047, 1272086876638937130):
                embed = e.get_embed(':warning: 이 커맨드는 도서관 채널에서만 사용 가능합니다!', error=True)
                embed.set_author(name=interaction.user.display_name, icon_url=avatar)
    ...
    async def return_book(self, interaction: discord.Interaction, book_id: str):
        ...
        try:
            ...
            if not rentable:
                ...
                embed = e.get_embed(f'[{book_id}] {title} {series}권을 반납하였습니다.',
                                    title='성공적으로 반납되었습니다!')
                embed.set_author(name=interaction.user.display_name, icon_url=avatar)
                await interaction.response.send_message(embed=embed)
            else:
    ...
    async def add_book(self, interaction: discord.Interaction, title: str, series: str,
                       location: str, category: app_commands.Choice[str], language: app_commands.Choice[str],
                       byname1: str = None, byname2: str = None):
        # Exception
        if interaction.user.avatar is not None:
            avatar = interaction.user.avatar.url
        else:
        ...
        if len(title) > 100:
            ...
            try:
                ...
                book.insert_book_data(title, series, byname1, byname2, location, category.value, language.value)
    ...
    async def delete_book(self, interaction: discord.Interaction, book_id: str):
        ...
        try:
            if book.get_info_by_id(book_id):
                ...
                embed = e.get_embed(f'[{book_id}] {title} ({series}권)를 삭제하였습니다.',
                                    title='성공적으로 삭제되었습니다!')
                embed.set_author(name=interaction.user.display_name, icon_url=avatar)
                await interaction.response.send_message(embed=embed)
            else:
    ...
    async def change_book(self, interaction: discord.Interaction, book_id: str,
                          change: app_commands.Choice[str], to: str):
        # Exception
        if interaction.user.avatar is not None:
            avatar = interaction.user.avatar.url
        else:
        ...
        if change.value == 'title' and book.get_info_by_id(book_id)[0][0] == to:
            ...
            await interaction.response.send_message(embed=embed)
            return
        elif change.value == 'location' and (len(to) > 4 or '-' not in to or to.upper() != to):
            embed = e.get_embed(':warning: 책장 번호를 다시 확인해주세요.', error=True)
            embed.set_author(name=interaction.user.display_name, icon_url=avatar)
            await interaction.response.send_message(embed=embed)
            return
            ...
            try:
                ...
                embed = e.get_embed(f'[{book_id}] {title} ({series}권)의 {change.value}가 {to}로 변경되었습니다.',
                                    title='성공적으로 추가되었습니다!')
                embed.set_author(name=interaction.user.display_name, icon_url=avatar)
                await interaction.response.send_message(embed=embed)
