...
async def 이누이(ctx, text=None):
    if text is None:
        await ctx.channel.send(
            f"{ctx.author.mention} 이누이 기여워 <a:InuiWaveYay:1272086403135832125>"
        )
    elif text == "과제해줘":
        await ctx.channel.send(
            f"{ctx.author.mention} 챗GPT쨩이 더 똑똑해요 <a:InuiWaveYay:1272086403135832125>"
        )
    elif text == "삼누삼":
        await ctx.channel.send(
            f"{ctx.author.mention} 사누사 오누오 <a:InuiWaveYay:1272086403135832125>"
        )
    elif text == "삼오라버니":
        await ctx.channel.send(
            f"{ctx.author.mention} 건좀 <a:InuiWaveYay:1272086403135832125>"
        )
    elif text == "생일":
        await ctx.channel.send(
            f"{ctx.author.mention} 9월 9일 <a:InuiWaveYay:1272086403135832125>"
        )
...
async def pome(interaction: discord.Interaction):
    urls = [
        "https://buly.kr/9iEWuT2",
        "https://buly.kr/FWRXHOa",
        "https://buly.kr/HHb75YY",
        "https://buly.kr/HSVs4NL",
        "https://buly.kr/4QloRrJ",
        "https://buly.kr/6Mq9EqV",
        "https://buly.kr/DPSSTTt",
    ]
...
async def search_books(
    interaction: discord.Interaction, title: str, series: str = None
):
...
async def rent_book_outer(
    interaction: discord.Interaction, name: discord.Member, book_id: str
):
    try:
        if interaction.channel_id not in (1280321483285074047, 1272086876638937130):
            embed = e.get_embed(
...
async def add_book(
    ...
    if len(title) > 100:
    ...
    elif len(category) > 5 or category not in [
        "만화",
        "소설",
        "작법서",
        "일러북",
        ...
        embed.set_footer(
            text="가능한 도서 분류는 '만화', '소설', '작법서', '일러북', '잡지' 입니다."
        )
        await interaction.response.send_message(embed=embed)
        return
        ...
        try:
            ...
            book.insert_book_data(
                title, series, byname1, byname2, location, category, language
            )
...
async def delete_book(interaction: discord.Interaction, book_id: str):
    ...
    try:
        if book.get_info_by_id(book_id):
            ...
            embed = e.get_embed(
                f"도서 {book_id}를 삭제하였습니다.", title="성공적으로 삭제되었습니다!"
            )
...
async def change_book(
    interaction: discord.Interaction, book_id: str, change: str, to: str
):
    role = discord.utils.get(interaction.user.roles, id=1181080308590858361)
    if role is None:
        embed = e.get_embed(":warning: 명령어 사용 권한이 없습니다.", error=True)
    ...
    if not book.get_info_by_id(book_id):
        ...
        embed = e.get_embed(
            ":warning: 수정하고자 하는 카테고리가 틀렸습니다.", error=True
        )
        embed.set_author(
            name=interaction.user.display_name, icon_url=interaction.user.avatar.url
        )
        embed.set_footer(
            text="가능한 카테고리는 byname1, byname2, location, category, language입니다."
        )
        await interaction.response.send_message(embed=embed)
        return
    ...
    # to는 너무 복잡해서 다음 업데이트에 추가
        ...
        try:
            ...
            embed = e.get_embed(
                f"[{book_id}]의 {change}가 {to}로 변경되었습니다.",
                title="성공적으로 추가되었습니다!",
            )
            embed.set_author(
                name=interaction.user.display_name, icon_url=interaction.user.avatar.url
...
async def keep_goods(interaction: discord.Interaction, name: str, goods: str):
    if interaction.channel_id not in (1279810905009426474, 1272086876638937130):
        ...
        member = discord.utils.find(
            lambda m: m.name == name or m.display_name == name, guild.members
        )
        ...
        if book.get_storage(name) is not None:
            await member.send(
                f"> {member.mention}님의 물품 보관 시간이 1일 남았습니다."
            )
